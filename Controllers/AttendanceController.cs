...
namespace Sector_13_Welfare_Society___Digital_Management_System.Controllers
{
    public class AttendanceController : Controller
    {
        ...
        public async Task<IActionResult> AllRecords(DateTime? from, DateTime? to, int? employeeId, string? status)
        {
            ...
            // Server-side computed balance summary
            var balanceSummary = attendances
                .GroupBy(a => a.EmployeeId)
                .Select(g => new
                {
                    EmployeeId = g.Key,
                    Present = g.Count(x => x.Status == "Present" || x.Status == "On-time" || x.Status == "Late"),
                    Absent = g.Count(x => x.Status == "Absent")
                })
                .ToDictionary(x => x.EmployeeId, x => new { x.Present, x.Absent });
        ...
        public async Task<IActionResult> DeleteConfirmed(int id)
        {
            ...
            if (attendance != null)
            {
                ...
                await _context.SaveChangesAsync();
                TempData["AttendanceDeleted"] = "Attendance record deleted.";
                return RedirectToAction("AllRecords", new { from = date.AddDays(-7), to = date.AddDays(7), employeeId = attendance.EmployeeId });
            }
